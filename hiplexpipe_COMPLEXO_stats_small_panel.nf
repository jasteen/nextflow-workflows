...
vardictBed       = file("/projects/vh83/reference/hiplex_complexo/COMPLEXO_missed_48.bed")
...
ch_inputFiles = Channel.fromPath("./bam_out/*sorted.bam").map{file -> tuple(file.name.take(file.name.lastIndexOf('.')), file)}
...
process CoverageBed {
    ...
    publishDir path: './metrics_testing', mode: 'copy'
    ...
    coverageBed -b ${bam} -a ${vardictBed} \
        -sorted -hist -g ${genome_file} > ${sample}.bedtools_metrics_all.txt
    """
}
