...
Channel.fromFilePairs("$inputDirectory/*_{R1,R2}.fastq.gz",flat: true).into{ch_inputFiles;ch_forFastqc}
...
process groupreadsByUmi {
    ...
    input:
        set baseName, file(bam), file(metrics) from ch_umiCorrectedBams
    output:
        set baseName, file("${baseName}.piped.corrected.grouped.histogram.tsv"), file("${baseName}.piped.corrected.grouped.bam") into ch_umiGroupedBams
    ...
    java -Xmx${task.memory.toGiga() - 2}g -Djava.io.tmpdir=$tmp_dir -jar $fgbioJar GroupReadsByUmi \
         -i ${bam} -f "${baseName}.piped.corrected.grouped.histogram.tsv" -o "${baseName}.piped.corrected.grouped.bam" -s paired -e 0 --min-map-q=20 
    """
...
process runVardict {
    ...
    export PATH=/home/jste0021/scripts/VarDict-1.5.8/bin/:$PATH
    VarDict -G ${ref} -f 0.01 -N "${tbam}|${nbam}" \
        -b "${tbam}|${nbam}" -th ${task.cpus} --nosv -UN -c 1 -S 2 -E 3 -g 4 ${segment} \
        > "${sample}.${ttype}_v_${ntype}.${segment}.somatic.vardict.tsv"
    """ 
