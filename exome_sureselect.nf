...
refBase          = "$refFolder/bwa_0.7.12_index/human_g1k_v37_decoy"
...
process surecallTrimmer {
    ...
    output:
        set baseName, file("${baseName}.trimmed.fastq.gz*") into processedInputFiles
    ...
    """
    java -Xmx{mem}g -jar surecalltrimmerJar \
       -fq1 ${fastqs[0]} -fq2 ${fastqs[1]} -xt -out_loc .  
    """
       
//  'mv ./{sample}_R1.fastq.gz* ./processed_fastqs/{sample}_R1.processed.fastq.gz; '
//  'mv ./{sample}_R2.fastq.gz* ./processed_fastqs/{sample}_R2.processed.fastq.gz' 
