...
process run_vardict {
    ...
    VarDict -G ${ref} -f 0.1 -N "${baseName}" --nosv -p -b ${bam} -c 1 -S 2 -E 3 -g 4 ${params.vardictBed} > "${baseName}.tsv"
    """
}
...
process mergeVCFS {
    ...
    for i in temp_shorter_*; do bcftools merge -m none -l \$i -O z -o \$i.merged.vcf.gz; bcftools index -t \$i.merged.vcf.gz; done
