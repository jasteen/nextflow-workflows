...
ch_inputFiles = Channel.fromPath("./mpileup/*.gz").map{file -> tuple(file.name.take(file.name.lastIndexOf('_')), file)}.take(5).subscribe{println it}
ch_inputIndexes = Channel.fromPath("./mpileup/*.gz.tbi").map{file -> tuple(file.name.take(file.name.lastIndexOf('_')), file)}take(5).subscribe{println it}
//ch_combined = ch_inputFiles.join(ch_inputIndexes).subscribe{println it}
