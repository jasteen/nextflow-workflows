...
process mergeVCFS {
    ...
    bcftools filter -i 'COUNT(GT!="ref")>COUNT(GT="mis")' merged.vardict.vcf.gz -O z -o merged.vardict.filtered.vcf.gz 
    """
}
...
process apply_vep {
    ...
    output:
        file("merged.vardict.filtered.vt.vep.vcf") into ch_vepVCF
    ...
    vep --cache --dir_cache $other_vep \
                      ...
                      --fork ${task.cpus} \
                      -i ${vcf} \
                      -o merged.vardict.filtered.vt.vep.vcf
    """
}
